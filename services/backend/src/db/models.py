...
class Candidate(BaseModel):
    ...
    status: Mapped[str] = mapped_column(Enum(CandidateStatus, values_callable=lambda x: [e.value for e in x], name="candidatestatus", create_type=False), default=CandidateStatus.ACTIVE.value)
...
class Resume(BaseModel):
    ...
    embedding: Mapped[Optional[List[float]]] = mapped_column(Vector(3072), nullable=True)
    parsed_data: Mapped[Optional["ParsedResume"]] = relationship("ParsedResume", back_populates="resume", uselist=False, cascade="all, delete-orphan")

class ParsedResume(BaseModel):
    __tablename__ = "parsed_resumes"
    resume_id: Mapped[UUID] = mapped_column(ForeignKey("resumes.id", ondelete="CASCADE"), nullable=False)
