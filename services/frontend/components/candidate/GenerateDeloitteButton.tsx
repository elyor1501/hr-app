...
import { generateDeloitteResume, deleteDeloitteResume } from "@/lib/candidates/action";
...
export function GenerateDeloitteButton({ candidateId, cvId, deloittePptxUrl, cvFileName, onSuccess }: Props) {
  ...
  const handleView = () => {
    ...
    window.open(`https://docs.google.com/gview?url=${encodeURIComponent(deloittePptxUrl)}&embedded=true`, "_blank");
  };
      ...
      <div className="flex items-center gap-2">
        ...
        <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
          ...
          <DialogContent className="w-[calc(100%-2rem)] sm:w-auto max-w-lg mx-auto rounded-xl sm:rounded-lg">
            <DialogHeader>
              <DialogTitle style={{ color: "#429ABD" }}>Delete Deloitte Resume</DialogTitle>
              <DialogDescription>Are you sure you want to delete this Deloitte resume?</DialogDescription>
            </DialogHeader>
            <div className="flex flex-col sm:flex-row justify-end gap-2 mt-4">
              <Button variant="outline" onClick={() => setDeleteOpen(false)} disabled={isPending} className="w-full sm:w-auto">Cancel</Button>
              <Button variant="destructive" onClick={handleDelete} disabled={isPending} className="w-full sm:w-auto bg-red-600 hover:bg-[#F5A623]">
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => { if (!generating) e.currentTarget.style.backgroundColor = "#F5A623"; }}
      onMouseLeave={(e) => { if (!generating) e.currentTarget.style.backgroundColor = "#429ABD"; }}
