...
async def test_ai_client_circuit_breaker_opens(respx_mock):
    ...
    # fail_max=5, so we need 5+ failures to open the breaker
    # Each call = 1 failure (no retry on HTTPStatusError)
    # We call 6 times to ensure breaker opens
    for i in range(6):
        try:
    ...
    # Check breaker is open by checking the state type
    ...
    # Next call should fail immediately via Breaker
